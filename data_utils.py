...
def encontrar_coluna(colunas: pd.Index, final_nome: str) -> str | None:
    ...
    return next(
        (coluna for coluna in colunas if str(coluna).lower().endswith(final_normalizado)),
        None,
    )


def converter_data_nascimento(valor: object, referencia: pd.Timestamp) -> pd.Timestamp:
    """Converte formatos de data encontrados nas planilhas de exportação."""
    if pd.isna(valor):
        return pd.NaT
    if isinstance(valor, (pd.Timestamp, date)):
        ...
        texto_normalizado = unicodedata.normalize("NFKD", texto).encode("ascii", "ignore").decode().lower()
        numerico = re.fullmatch(r"(\d{2})(\d{2})(\d{4})", texto_normalizado)
        semi_numerico = re.fullmatch(r"(\d{1,2})\s+(\d{2})(\d{4})", texto_normalizado)
        curta = re.fullmatch(r"(\d{1,2})[./\s]+(\d{1,2})[./\s]+(\d{2,4})", texto_normalizado)
        extenso = re.fullmatch(
            r"(\d{1,2})\s+(?:de\s+)?([a-z]+)(?:\s+de)?\s*-?\s*(\d{4})",
            texto_normalizado,
        )
        if numerico or semi_numerico or curta:
            dia, mes, ano = map(int, (numerico or semi_numerico or curta).groups())
            if curta and mes > 12 and dia <= 12:
                dia, mes = mes, dia
            if ano < 100:
                ano = 2000 + ano
                if ano > referencia.year:
                    ano -= 100
            data = pd.to_datetime(f"{dia:02d}/{mes:02d}/{ano}", format="%d/%m/%Y", errors="coerce")
        elif extenso and extenso.group(2) in MESES:
            dia, mes, ano = int(extenso.group(1)), MESES[extenso.group(2)], int(extenso.group(3))
            data = pd.to_datetime(f"{dia:02d}/{mes:02d}/{ano}", format="%d/%m/%Y", errors="coerce")
        else:
...
def preparar_dados(df: pd.DataFrame, referencia: pd.Timestamp | None = None) -> pd.DataFrame:
    ...
    nascimento = (
        resultado[coluna_data].map(lambda valor: converter_data_nascimento(valor, referencia))
        if coluna_data
        else pd.Series(pd.NaT, index=resultado.index, dtype="datetime64[us]")
    )
    idade_data = pd.Series(pd.NA, index=resultado.index, dtype="Int64")
    tem_data = nascimento.notna()
    idade_data.loc[tem_data] = (
        referencia.year
        - nascimento.loc[tem_data].dt.year
        - (
            (referencia.month < nascimento.loc[tem_data].dt.month)
            | ((referencia.month == nascimento.loc[tem_data].dt.month) & (referencia.day < nascimento.loc[tem_data].dt.day))
        ).astype(int)
    ).astype("Int64")
    idade_ano = pd.Series(pd.NA, index=resultado.index, dtype="Int64")
    if coluna_ano:
        anos = pd.to_numeric(resultado[coluna_ano], errors="coerce")
        validos = anos.notna() & anos.eq(anos.round()) & anos.between(1900, referencia.year)
        idade_ano.loc[validos] = (referencia.year - anos.loc[validos]).astype("Int64")
    resultado["Idade"] = idade_data.combine_first(idade_ano)
    resultado["Origem da idade"] = "Não informado"
    resultado.loc[idade_ano.notna(), "Origem da idade"] = "Ano de nascimento"
    resultado.loc[idade_data.notna(), "Origem da idade"] = "Data de nascimento"
...
def resumo_tcle(df: pd.DataFrame) -> tuple[int, int]:
    ...
    if coluna is None:
        return 0, 0
    respostas = df[coluna].map(normalizar_texto)
    verdadeiros = {"true", "1", "sim", "s", "yes", "y", "verdadeiro"}
    falsos = {"false", "0", "nao", "no", "n", "f", "falso"}
    return int(respostas.isin(verdadeiros).sum()), int(respostas.isin(verdadeiros | falsos).sum())
